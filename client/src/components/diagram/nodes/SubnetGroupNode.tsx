...
function SubnetGroupNode({ data, selected }: NodeProps) {
  ...
  const displayLabel = truncateLabel(d.label);
  ...
  return (
    <div className="subnet-group-node" style={style}>
      ...
      />
      <div className="subnet-label" style={labelStyle} title={d.label}>
        {d.favourite && <span style={{ marginRight: '0.3rem' }}><FavouriteStar size="1em" /></span>}
        {displayLabel} ({d.cidr})
      </div>
